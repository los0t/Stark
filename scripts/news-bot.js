...
const CONFIG = {
  ...
  RSS_FEEDS: [
    ...
    {
      url: 'https://www.nytimes.com/services/xml/rss/nyt/Business.xml',
      source: 'The New York Times Business'
    },
    {
      url: 'https://www.nytimes.com/services/xml/rss/nyt/Technology.xml',
    ...
    {
      url: 'https://news.google.com/rss/search?q=interesting+world+news&hl=en-US&gl=US&ceid=US:en',
      source: 'Google News'
    },
    {
      url: 'https://news.google.com/rss/search?q=unusual+world+news&hl=en-US&gl=US&ceid=US:en',
      source: 'Google News'
    }
  ]
};
...
この候補の中から、
日本の若者が「何それ！？」「詳しく知りたい」と思うような
最も興味深い出来事を1つだけ選んでください。

そして、その出来事についてEcstasy用の記事を1記事作成してください。
...
日本のニュースも候補に含まれている場合は、
日本・海外を問わず最も興味深い出来事を選んでください。
...
1記事につき最低1つの出典を付けてください。

出典URLは、
必ず候補として与えられたURLの中から選んでください。
...
async function main() {
  ...
  /* =======================================================
     ...
     ※ここでGemini APIを1回だけ呼び出す
     ※関連記事検索の2回目は行わない
  ======================================================= */

  console.log(
    `🤖 ${allCandidates.length}件から最も興味深い出来事を選択し、記事を生成中...`
  );
